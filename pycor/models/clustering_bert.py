...
def do_clustering(data: pd.DataFrame, wcl='[]'):
    ...
    for name, group in data.groupby(['lemma', 'ordklasse', 'homnr']):
        ...
        # pairs = [Pair(bets=(name[0], name[1]),
        #               distance=group.score.iloc[0],
        #               label=group.label.iloc[0]) for name, group in group.groupby(['bet_1', 'bet_2'])
        #          ]
        #
        ...
        if len(uniq_pairs) == 1:
            ...
            if x.distance >= 0.8 else 1 - x.distance + 0.95
            ...
            for pair in pairs:
                ...
                if pair.distance >= 0.74:
                    ...
                    elif pair.distance in minimums:
                        clusters = split(clusters, inv_clusters, pair, uniq_pairs)
